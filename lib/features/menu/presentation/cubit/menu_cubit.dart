...
class MenuCubit extends Cubit<MenuState> {
  ...
  Future<void> getMenu({required int page}) async {
    ...
    final result = await getMenuItemsUseCase(page: page);
    ...
    result.fold(
        ...
        if (newProducts.isEmpty) {
          ...
          emit(state.copyWith(
            ...
            hasReachedMax: false, // Ensure we can still try to fetch the next page later
          ));
        }
      },
    );
  ...
  Future<void> searchMenu(String query) async {
    ...
    final result = await searchMenuItemsUseCase(query: query);
